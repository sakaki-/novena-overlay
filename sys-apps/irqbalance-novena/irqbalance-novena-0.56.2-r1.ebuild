...
inherit eutils autotools versionator systemd
...
src_install() {
	default
	newinitd "${FILESDIR}"/irqbalance.init-0.55-r3 irqbalance
	newconfd "${FILESDIR}"/irqbalance.confd irqbalance
	systemd_newunit debian/irqbalance-imx.service irqbalance.service 
}
...
pkg_postinst() {
	if [[ -z ${REPLACING_VERSIONS} ]]; then
		...
		if has_version sys-apps/openrc; then
			...
			# assume systemd
			elog "-- # systemctl enable irqbalance"
			elog "-- # systemctl start irqbalance"
		fi
	fi
}
